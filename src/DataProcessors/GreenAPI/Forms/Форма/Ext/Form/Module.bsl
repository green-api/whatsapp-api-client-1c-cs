...
Процедура ОтправитьСообщениеWhatsApp()
	...
	Если ПереключательТипаЧата = 1 Тогда
		Ответ = ОбработкаОбъект().ОтправитьТекстВГруппу(ПараметрИдЧата, ОтправкаТекстСообщения);
	Иначе
...
Процедура СоздатьОпрос(Вопрос, ВариантыОтветов, НесколькоВариантов)
	...
	Если ПереключательТипаЧата = 1 Тогда
		Ответ = ОбработкаОбъект().ОтправитьОпрос(ПараметрИдЧата, Вопрос, ВариантыОтветовМассив, НесколькоВариантов);	
	Иначе
...
Процедура ВыбратьСсылкуДляОтправкиСервер(АдресФайла, ИмяФайла)
	...
	Если ПереключательТипаЧата = 1 Тогда
		Ответ = ОбработкаОбъект().ОтправитьВидеоАудиоИзображениеДокументПоURL(ПараметрИдЧата, АдресФайла, ИмяФайла, ОтправкаТекстСообщения);	
	Иначе
...
Функция ПроверитьПодключение(Знач ПоказатьСообщение = Истина)	
	...
	Результат = (НРег(СтатусСервиса) = "авторизирован");	
...
Процедура ПроверитьПодключениеКоманда(Команда)	
	...
	ПоказатьСообщениеВыполненияКоманды(СтатусСервиса = "Авторизирован", СтруктураСообщения);	
...
Процедура ОтправитьСообщение(Команда)	
	...
	Если Не ЗначениеЗаполнено(НомерТелефона) Или Не ЗначениеЗаполнено(ПараметрИдЧата) Тогда
		
		Если ПереключательТипаЧата = 1 Тогда			
			ПоказатьСообщениеПользователю("Поле ""ID чата"" не заполнено", "ПараметрИдЧата");
		Иначе                                                                                
...
Процедура УстановитьСтандартныеНастройкиПараметрыХостов(Команда)			
	...
	ТекстВопроса = "Установить стандартные настройки хостов?";
